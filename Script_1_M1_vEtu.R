...
# DATASET : Voir Git-Hub 

# Mot de passe du dataset : 
x


#### ____ Introduction au système de packages ____ ####

# Pour télécharger un tout nouveau package sur l'ordinateur (e.g., prettyR) : 

#         install.packages("ggplot2") # A REALISER QU'UNE SEULE FOIS SUR L'ORDINATEUR

# Une fois téléchargé dans la librairie, il faut l'appeler :

library(ggplot2) # A chaque ouverture de session de RStudio, il faudra appeler le packages si on veux utiliser ses fonctions

#### ____ 1. Charges ses propres données ____ ####

dt = read.csv("Dataset/Dataset_M1csv.csv",
              ...
              na.strings=c("","NA"))    # Pour spécifier les cellules vides (non obligatoire)       
...
# R est un langage objet
# L'objet " dt " est une matrice comprenant nos données
...
Indiv67 = dt[dt$PatientId==67 , ]      # Attention, la recherche de texte requière des guillemets : dt$COLONNE == "texte"
Indiv67
...
# Je peux également identifier s'il manque des données "NA"

missing_HG = is.na(dt$HandgripN)    # TRUE = Missing, FALSE = Present
table(missing_HG)["FALSE"]          # How many available data ?


#### ---- EXERCICE n°1 | Introduction aux conditions logiques---- ####

# Question 1
# Combien y a t-il d'observations de femmes dans la base de données ?


# Combien y a t-il d'observations de femmes de plus de 80 ans ? 


# Combien y a t-il d'observations d'hommes de plus de 80 ans et avec un IMC >= 35


# Combien y a t-il d'observations avec des niveau d'activité physique "Small" OU "Medium" ?


#### ____ 3. Décrire ses données ____ ####

# Pour décrire ses données, on peut utiliser des paramètres de position
...
quantile(dt$BMI, na.rm = TRUE)     # na.rm = TRUE obligatoire, car aucune valeur manquante n'est autorisée
...
# Pour calculer une information en fonction d'un groupe
...
## Comparez la force MOYENNE de Handgrip entre les femmes âgées (>=60 ans) et les femmes plus jeunes ? Que concluez vous ?
